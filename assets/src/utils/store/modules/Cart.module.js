...
export default {
    ...
    getters: {
        ...
        getOrderTotal(state, getters) {
            return (
                getters.getSubtotal + getters.getTotalFee + getters.getTotalTax
            );
        },
    ...
    mutations: {
        ...
        addToCartItemWithQuantity(
            state,
            { product, quantity = 1, line_item_id = 0 }
        ) {
            var cartObject = {};
            ...
            if (line_item_id) {
                cartObject.id = line_item_id;
                cartObject.total =
                    quantity * parseInt(`${product.sales_display_price}`, 10);
            }
        ...
        addToCartItem(state, product, quantity = 1) {
            var cartObject = {};
            cartObject.product_id =
                product.parent_id === 0 ? product.id : product.parent_id;
            cartObject.name = product.name;
        ...
        toggleEditQuantity(state, itemKey) {
            state.cartdata.line_items[itemKey].editQuantity =
                !state.cartdata.line_items[itemKey].editQuantity;
        },
    ...
    actions: {
        ...
        addToCartActionWithQuantity(
            context,
            { product, quantity, line_item_id }
        ) {
            context.commit("addToCartItemWithQuantity", {
                product,
                quantity,
                line_item_id,
            });
